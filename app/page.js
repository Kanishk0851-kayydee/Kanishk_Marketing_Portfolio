...
export default function Home() {
  ...
  // Track scrolling to trigger the fade effect
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FCFBF9] text-gray-900 font-sans pb-24 selection:bg-gray-200">
      ...
      <div className={`fixed top-0 left-0 w-full z-40 transition-all duration-500 ease-in-out ${isScrolled ? 'bg-[#FCFBF9]/95 backdrop-blur-md py-4 border-b border-gray-200 shadow-sm' : 'bg-transparent py-8'}`}>
        <nav className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          {/* Fading Logo */}
          <div className={`text-xl md:text-2xl font-bold font-serif tracking-tight transition-opacity duration-500 ${isScrolled ? 'opacity-100 cursor-pointer' : 'opacity-0 pointer-events-none'}`} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            Kanishk Dawar
          </div>
      ...
      {/* Hero Section */}
      <section className="flex flex-col items-center text-center pt-48 pb-20 px-6 max-w-4xl mx-auto min-h-[70vh] justify-center">
        <img 
          src="/kanishk-headshot.jpg.jpeg" 
          alt="Kanishk Dawar" 
          className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover mb-8 shadow-md grayscale hover:grayscale-0 transition-all duration-500"
        />
      ...
      {/* About Section */}
      <section id="about" className="max-w-5xl mx-auto mt-20 px-6 grid md:grid-cols-2 gap-16 items-center">
        <div>
      ...
      {/* Footer / CTA */}
      <section className="max-w-3xl mx-auto mt-32 mb-12 px-6">
        <h2 className="text-3xl font-serif font-bold mb-4 text-gray-900">Let's Connect</h2>
        <p className="text-gray-600 mb-8 text-sm text-justify">
